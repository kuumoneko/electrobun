            ...
            if (g_asarArchive)
            {
                ...
                if (!data_.empty())
                {
                    ...
                    if (fullPath.find(".html") != std::string::npos)
                        ...
                        mimeType = "image/jpeg";
                    else if (fullPath.find(".svg") != std::string::npos)
                        mimeType = "image/svg+xml";
                    else if (fullPath.find(".woff") != std::string::npos)
                        mimeType = "font/woff";
                    else if (fullPath.find(".woff2") != std::string::npos)
                        mimeType = "font/woff2";
...
public:
    ElectrobunClient(uint32_t webviewId,
                     ...
                     GtkWidget *gtkWidget,
                     bool sandbox)
        : webview_id_(webviewId), event_bridge_handler_(eventBridgeHandler), bun_bridge_handler_(bunBridgeHandler), webview_tag_handler_(internalBridgeHandler), webview_event_handler_(webviewEventHandler), navigation_callback_(navigationCallback), is_sandboxed_(sandbox), gtk_widget_(gtkWidget), x11_window_(0), display_(nullptr), osr_enabled_(false), osr_width_(0), osr_height_(0), parent_window_handle_(0) {}
    ...
    {
        if (event.type == KEYEVENT_KEYDOWN && event.windows_key_code == 123)
        { // F12 key
                                          ...
                                          CefRefPtr<CefProcessMessage> message) override
    {
        ...
        g_timeout_add(1000, [](gpointer data) -> gboolean
                      {
            free(data);
            return G_SOURCE_REMOVE; }, contentCopy);
    ...
    void remove() override
    {
        ...
        if (webview)
        {
            ...
            g_idle_add([](gpointer data) -> gboolean
                ...
                return G_SOURCE_REMOVE; }, widget_to_destroy);
        }
    }
...
public:
    ...
    void remove() override
    {
        ...
        if (browser_to_close)
        {
            ...
            g_idle_add([](gpointer data) -> gboolean
                ...
                delete browser_ref;
                return G_SOURCE_REMOVE; }, new CefRefPtr<CefBrowser>(browser_to_close));
        }
    ...
    void resize(const GdkRectangle &frame, const char *masksJson) override
    {
        ...
        if (browser)
        {
            ...
            if (adjustedFrame.width < 0)
                adjustedFrame.width = 0;
            if (adjustedFrame.height < 0)
                adjustedFrame.height = 0;
...
static gboolean onWindowDeleteEvent(GtkWidget *widget, GdkEvent *event, gpointer user_data)
{
    ...
    g_idle_add_full(G_PRIORITY_HIGH, [](gpointer data) -> gboolean
        ...
        return G_SOURCE_REMOVE; }, widget, nullptr);
...
GtkWidget *createMenuFromParsedItems(const std::vector<MenuJsonValue> &items, ZigStatusItemHandler clickHandler, uint32_t trayId)
{
    ...
    for (const auto &item : items)
    {
        ...
        else
        {
            ...
            std::string displayLabel = !item.label.empty() ? item.label : (!item.role.empty() ? item.role : "Menu Item");
            ...
            itemData->action = !item.action.empty() ? item.action : (!item.role.empty() ? item.role : "");
...
GtkWidget *createApplicationMenuBar(const std::vector<MenuJsonValue> &items, ZigStatusItemHandler clickHandler)
{
    ...
    for (const auto &item : items)
    {
        ...
        else
        {
            ...
            std::string displayLabel = !item.label.empty() ? item.label : (!item.role.empty() ? item.role : "Menu");
        ...
        if (g_asarArchive)
        {
            ...
            // Protect ASAR read operations with mutex
            {
                ...
                if (fileData && asarFileSize > 0)
                {
                    ...
                    fileContents = (gchar *)g_memdup(fileData, asarFileSize);
...
static void setX11WindowIcon(X11Window *x11win, GdkPixbuf *pixbuf)
{
    if (!x11win || !x11win->display || !x11win->window || !pixbuf)
        return;
...
extern "C"
{
    ...
    void resizeAutoSizingWebviewsInWindow(uint32_t windowId, int width, int height)
    {
        ...
        std::vector<std::pair<uint32_t, std::shared_ptr<AbstractView>>> oopifWebviews;
        {
            ...
            for (auto &[webviewId, webview] : g_webviewMap)
            {
                if (webview)
                {
                    ...
                    bool belongsToWindow = (wgpuView && wgpuView->parentXWindow == x11WindowHandle);
                    if (belongsToWindow)
                    {
                        if (webview->fullSize)
                        {
        ...
        // (CEF-specific, disabled without CEF)
    }
                                              ...
                                              g_signal_connect(window, "key-press-event", G_CALLBACK(+[](GtkWidget *widget, GdkEventKey *event, gpointer user_data) -> gboolean
                                                                                                     {
                                                                                                         ...
                                                                                                         if (container && container->keyCallback)
                                                                                                         {
                                                                                                             ...
                                                                                                             if (event->state & GDK_SUPER_MASK)
                                                                                                                 modifiers |= (1 << 3); // Super/Windows key
                                 ...
                                 bool startPassthrough)
    {
        ...
        try {
            ...
            webview = std::make_shared<WebKitWebViewImpl>(
                webviewId, static_cast<GtkWidget*>(window),
...
//    {
//        if (p_wgpuInstanceCreateSurface && p_wgpuSurfaceConfigure && p_wgpuSurfaceGetCurrentTexture && p_wgpuSurfacePresent && p_wgpuQueueOnSubmittedWorkDone && p_wgpuBufferMapAsync && p_wgpuInstanceWaitAny && p_wgpuBufferGetMappedRange && p_wgpuBufferUnmap)
//        {
//            return true;
//        }
...
//        }
//        if (outWidth)
//            *outWidth = (uint32_t)std::max(1, attrs.width);
//        if (outHeight)
//            *outHeight = (uint32_t)std::max(1, attrs.height);
//        return true;
//    }
//
...
//    {
//        if (!state || !state->surface || !state->device || !state->display || !state->window)
//            return;
//
...
//    {
//        if (!state || !state->device || !state->surface || !state->queue || !state->pipeline)
//            return;
//
//        uint32_t width = 1;
//        uint32_t height = 1;
...
//    }
    ...
    static KeySym getKeySym(const std::string &key)
    {
        ...
        if (lowerKey[0] == 'f' && lowerKey.length() >= 2)
        {
            ...
            if (fNum >= 1 && fNum <= 24)
            {
                if (fNum <= 12)
                    ...
                    return XK_F13 + (fNum - 13); // F13-F24
            }
        }
        // Special keys
        if (lowerKey == "space" || lowerKey == " ")
            return XK_space;
        ...
        if (lowerKey == "menu")
            return XK_Menu;
        if (lowerKey == "apps")
            return XK_Menu; // Same as Menu
        if (lowerKey == "printscreen")
            return XK_Print;
        if (lowerKey == "cancel")
            return XK_Cancel;
        // Media keys (may not be available on all systems)
        if (lowerKey == "mediaselect")
            return XK_Select; // Closest equivalent
        if (lowerKey == "calculator")
            return 0x1008ff1d; // XF86Calculator
        if (lowerKey == "sleep")
            return 0x1008ff2f; // XF86Sleep
        // Symbols
        if (lowerKey == "-")
            return XK_minus;
        if (lowerKey == "=")
    ...
    static unsigned int parseX11Modifiers(const std::string &accelerator, std::string &outKey)
    {
        ...
        if (parts.commandOrControl || parts.command || parts.control)
    ...
    static void onGetCookiesFinished(GObject *source, GAsyncResult *result, gpointer user_data)
    {
        ...
        if (!error && cookies)
        {
            ...
            while (item)
            {
                ...
                if (expires)
                {
                    json << ",\"expirationDate\":" << soup_date_to_time_t(expires);
                }
    ...
    ELECTROBUN_EXPORT bool sessionSetCookie(const char *partitionIdentifier, const char *cookieJson)
    {
        ...
        if (expirationDate > 0) {
            SoupDate* expires = soup_date_new_from_time_t((time_t)expirationDate);
